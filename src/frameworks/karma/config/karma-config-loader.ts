...
import { BrowserHelperProvider } from '../../../core/config/browsers/browser-helper-provider.js';
...
export class KarmaConfigLoader {
  ...
  private applyConfigOverrides(config: KarmaConfig, karmaConfigHomePath: string, karmaConfigPath?: string) {
    ...
    const autoWatchBatchDelay = !autoWatchEnabled
      ? 0
      : !Number.isNaN(configuredAutoWatchBatchDelay) && configuredAutoWatchBatchDelay >= 0
    ...
    if (requestedBrowser) {
      ...
      customLauncher =
        browserHelper.getCustomLauncherWithDebugPort(customLauncherObject, debugPort) ?? customLauncherObject;

      browser = KARMA_CUSTOM_LAUNCHER_BROWSER_NAME;

      this.logger.debug(() => `Using custom launcher: ${JSON.stringify(customLauncher, null, 2)}`);
    }
